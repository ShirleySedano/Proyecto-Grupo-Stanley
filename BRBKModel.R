...
BRBK<-function(){
  ...
  library(rugarch)
  library(readr)
  delay <- read_csv("C:/Users/aparra/Desktop/EntregaFinalMaestria/BRKStock/delay.csv")
  ...
  nombreAcciones<-c("BRK-B")
  acciones<-getSymbols(nombreAcciones, src = "yahoo", from =inicio, to = fin, periodicity = "daily")
  CierreAjustado<- cbind(`BRK-B`[,6])
  
  ###Calcular retornos diarios
  Rendimientos<-(periodReturn(CierreAjustado$`BRK-B.Adjusted`, period = "daily",type="log")*100)
